...
void CAN0_Handler(void)
{
    ...
    if (STATUS_INTERRUPT == status )  /* if interrupt is a Status interrupt */
    {
        ...
        /* clear the RXOK,TXOK,LEC bits in status register */
        CLEAR_BIT(REG_VAL(CAN0_BASE,CAN_STS_OFFSET) , RXOK_BIT);
        CLEAR_BIT(REG_VAL(CAN0_BASE,CAN_STS_OFFSET) , TXOK_BIT);
        CLEAR_BIT(REG_VAL(CAN0_BASE,CAN_STS_OFFSET) , LEC0_BIT);
        CLEAR_BIT(REG_VAL(CAN0_BASE,CAN_STS_OFFSET) , LEC1_BIT);
        CLEAR_BIT(REG_VAL(CAN0_BASE,CAN_STS_OFFSET) , LEC2_BIT);
        Error_Flag =1 ;  /* raise the error flag to detect that an error has occurred */
    }
    /* interrupt is a Message Object interrupt */
    else
    {
        MSG_Number_INT[status-ONE_VALUE] = status; /* store the message number that caused the interrupt in a variable */
        HOH=INIT_VAL_ZERO;
        uint8 Mailbox_Index;
        uint8 iter=Can_Configuration.CanConfigSet.CanHardwareObject[HOH].CanHardwareObjectCount;
        while( MSG_Number_INT[status-ONE_VALUE] > iter )
        {
            HOH++;
            iter+=Can_Configuration.CanConfigSet.CanHardwareObject[HOH].CanHardwareObjectCount;
        }
        for (iter = INIT_VAL_ZERO ; iter< Can_Configuration.CanConfigSet.CanHardwareObject[HOH].CanHardwareObjectCount ; iter++)
        {
            if (Object_Check[CAN0_CONTROLLER_ID][HOH][iter].Mailbox == MSG_Number_INT[status-ONE_VALUE])
            {
                Mailbox_Index=iter;
            }
            else
         ...
         */
        REG_VAL(CAN0_BASE, CAN_IF1CRQ_OFFSET) = MSG_Number_INT[status-ONE_VALUE] & MNUM_MASK;
        /* Wait for busy bit to clear */
        while(BIT_IS_SET(REG_VAL(CAN0_BASE, CAN_IF1CRQ_OFFSET),BUSY_BIT))
        {
            /*Do Nothing*/
