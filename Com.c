...
void Com_MainFunctionRx(void)
{
    ...
    else
    {
        ...
        {
            check_flag=0;
            uint8 signal_index_in_signal_buffer=Com.ComIPdu[check_flag].ComIPduSignalRef[check_flag]->ComHandleId;
            signal_per_pdu=Com.ComIPdu[check_flag].ComIPduSignalRef[check_flag];
            for(i=0;i<PDU[check_flag].SduLength;i++){
                SignalObject[signal_index_in_signal_buffer]=(PDU[check_flag].SduDataPtr)[i];
